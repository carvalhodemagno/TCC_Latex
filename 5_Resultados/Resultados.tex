...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Neste capítulo são apresentados os resultados obtidos a partir da abordagem descrita no capítulo \ref{chapter:desenvolvimento}, levando em conta o processo pós desenvolvimento, os testes com o sistema integrado e a avaliação de desempenho conforme os objetivos específicos. 

Com a confecção da PCB a partir do \textit{layout} e a adquisição dos componentes eletrônicos, conforme a figura \ref{fig:layout_pcb} e a tabela \ref{table:lista_material} respectivamente, foi possível realizar o processo soldagem manual, com intuito de validar o \textit{hardware} desenvolvido. Em seguida, foram executados os testes elétricos e funcionais na placa já montada, representada pela figura \ref{fig:pcb_montada_top}, o que resultou na alteração de alguns componentes, como capacitores, visando assim aperfeiçoar o funcionamento da placa.	
...
Em seguida, o \textit{firmware} desenvolvido foi embarcado no processador já soldado na placa, por meio do conector de gravação, "P3"\,da figura \ref{fig:Schematic_Core_Processor}, e um conversor USB/Serial conectado ao computador.
...
É possível observar uma representação gráfica, nas figuras \ref{fig:grafios_medicao_tensão}, \ref{fig:graficos_medicao_corrente}, \ref{fig:graficos_medicao_frequencia} e \ref{fig:graficos_medicao_potencia}, do comparativo entre as medições realizadas pelo protótipo e pelo alicate amperímetro juntamente com os erros relativos e absolutos, gerada a partir dos dados contidos nas tabelas do Apêndice \ref{apendice:medicoes}. 
